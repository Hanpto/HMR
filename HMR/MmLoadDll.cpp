...
void ShowError(CHAR* lpszText)
{
	...
	sprintf_s(szErr, MAX_PATH,"%s Error!\r\nError Code Is:%d\r\n", lpszText, GetLastError());
...
LPVOID MmLoadLibrary(LPVOID lpData, DWORD dwSize)
{
	...
	{
		ShowError((CHAR*)"DoRelocationTable");
		return NULL;
	}
...
LPVOID MmGetProcAddress(LPVOID lpBaseAddress, PCHAR lpszFuncName)
{
	...
	for (i = 0; i < dwNumberOfNames; i++)
	{
		...
		if (0 == lstrcmpi((LPCSTR)lpFuncName, (LPCSTR)lpszFuncName))/*****************这里要注意名称单双字问题**************************/
		{
			// 获取导出函数地址
